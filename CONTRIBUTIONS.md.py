...
def pull_request2markdown(pull_request):
    ...
    return f"""[{pull_request['repository']['nameWithOwner']}]({pull_request['url']}) {pull_request['changedFiles']} files changed, {pull_request['additions']} insertions(+), {pull_request['deletions']} deletions(-) -- {pull_request['title']}"""
